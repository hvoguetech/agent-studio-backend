...
from ros.models import Tool, ToolSetMember
from ros.secrets.store import SecretStore
from ros.tools.graphql import execute_graphql
from ros.tools.projection import estimate_tokens
from ros.tools.rest import execute_rest, project_observation
...
class ToolService:
    ...
    @staticmethod
    async def ensure_builtins(session: AsyncSession, tenant_id: str, project_id: str) -> None:
        """Provision the platform's built-in tools into a project if any are missing. Idempotent:
        only creates builtins whose `config.builtin` isn't already present, so it's safe to call
        on every project create and every tools-list read. Built-ins are project defaults - always
        available, protected from deletion, and excluded from import/export bundles."""
        from ros.tools.builtin import BUILTIN_DEFAULTS

        rows = await session.execute(
            select(Tool).where(
                Tool.tenant_id == tenant_id, Tool.project_id == project_id, Tool.kind == "builtin"
            )
        )
        have = {(t.config or {}).get("builtin") for t in rows.scalars()}
        to_add = [d for d in BUILTIN_DEFAULTS if d["builtin"] not in have]
        if not to_add:
            return
        for d in to_add:
            session.add(Tool(
                tenant_id=tenant_id, project_id=project_id, name=d["builtin"], kind="builtin",
                config={"builtin": d["builtin"], "description": d["description"]},
            ))
        ...
        try:
            if kind == "rest_api":
                ...
                if kind == "builtin":
                    ...
                    from ros.tools.code import build_code_tool
                    tool = build_code_tool(cfg, ctx)
                else:
                    from ros.tools.sql import build_sql_tool
