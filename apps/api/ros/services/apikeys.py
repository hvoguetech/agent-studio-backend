...
Unlike the single static ROS_SERVICE_API_TOKEN (one identity, whole install), these are
...
Presented as `Authorization: Bearer <key>` and resolved in `ros.deps.get_current_user`.
"""
...
from ros.services.auth import ROLES

_KEY_PREFIX = "ros_sk_"        # recognizable, non-JWT, non-service-token marker
...
_PAT_PREFIX = "ros_pat_"       # per-user MCP personal access token (NOT a general-API key)
...
class ApiKeyService:
    ...
    async def create_personal(
        ...
        """Mint a per-user PAT (prefix ros_pat_) for MCP. Bound to `user_id`, optionally scoped to
    ...
    async def resolve_personal(session, presented: str) -> ApiKey | None:
        ...
        Only the ros_pat_ prefix is accepted (a general ros_sk_ key is never a PAT)."""
