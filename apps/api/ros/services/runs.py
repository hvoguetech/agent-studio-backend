...
LangGraph graph (with the app checkpointer + a ROSTracer), `astream` it, push
...
log = logging.getLogger("ros.runs")

# Settings wanted (config.py is owned by a parallel agent; module constants used for now):
#   ROS_HITL_APPROVAL_TIMEOUT_SECONDS (int, default 0 = never expire) - how long an
...
#   ROS_RUN_WALL_CLOCK_TIMEOUT_SECONDS (int, default 0 = unlimited) - hard per-run wall-clock
...
class _RunControl:
    ...
    def begin(self, run_id: str, tenant_id: str) -> asyncio.Event:
        ...
        self._watchers[run_id] = asyncio.create_task(
            self._watch_database(run_id, tenant_id), name=f"ros-cancel-watch:{run_id}",
        )
        ...
        self._heartbeats[run_id] = asyncio.create_task(
            self._heartbeat(run_id, tenant_id), name=f"ros-heartbeat:{run_id}",
        )
        return ev
...
async def _expand_hitl_decisions(graph, config, value):
    """Make a HITL resume tolerant of an AI turn that fires SEVERAL approval-gated tool calls at
    once. LangChain's HumanInTheLoopMiddleware raises ONE interrupt carrying every gated call in the
    turn, and on resume it requires exactly one decision per hanging call - otherwise it errors
    "Number of human decisions (N) does not match number of hanging tool calls (M)". Our approval
    UIs collect a single approve/reject for the whole batch, so replicate that decision across all
    pending calls. No-op unless `value` is the `{"decisions": [...]}` shape and the pending interrupt
    has more calls than decisions supplied."""
    if not (isinstance(value, dict) and isinstance(value.get("decisions"), list) and value["decisions"]):
        return value
    try:
        snap = await graph.aget_state(config)
    except Exception:  # noqa: BLE001 - best-effort; fall back to the value as sent
        return value
    count = 0
    for t in getattr(snap, "tasks", ()) or ():
        for it in getattr(t, "interrupts", None) or ():
            v = getattr(it, "value", None)
            reqs = v.get("action_requests") if isinstance(v, dict) else getattr(v, "action_requests", None)
            if reqs:
                count = max(count, len(reqs))
    decisions = value["decisions"]
    if count > len(decisions):
        decisions = decisions + [decisions[-1]] * (count - len(decisions))
        return {**value, "decisions": decisions}
    return value
...
# Sources that are ROS-internal (operator-driven), shown as "System" in the Traces view.
...
def _debug_nodes(definition: dict, tracer: ROSTracer) -> dict[str, dict]:
...
class RunService:
    ...
    async def create_run(
        ...
        if thread_id:
            ...
            # so the checkpointer holds no prior turns => the agent "rosts" the conversation.
            thread = (
                await session.execute(
                    select(Thread).where(
    ...
    async def _drive(
        ...
        async with SessionLocal() as session:
            ...
            # Live activity timeline: the tracer pushes a small frame per tool / sub-agent start &
            # end onto this thread-safe deque; the astream loop drains it to the broker as
            # `activity` events (operator-only). Lets the playground show where a deep_agent routed.
            ...
            try:
                async with tenant_concurrency.slot(tenant_id, settings.max_concurrent_runs_per_tenant), tlock:
                    ...
                    if resume:
                        resume_value = await _expand_hitl_decisions(graph, config, resume_value)
                    ...
                    async for ns, mode, chunk in graph.astream(
                        ...
                        if wall_clock and (time.monotonic() - started_monotonic) > wall_clock:
                            timed_out = True
                            break
                        # Flush any tool/sub-agent activity captured since the last frame (a live
                        # timeline of where the run routed). Operator-only, like node_start (H5).
                        while activity:
    ...
    async def run_to_completion(self, *, run_id: str, tenant_id: str, project_id: str | None = None, run_context: dict | None = None) -> dict:
        ...
        async with SessionLocal() as session:
            ...
            try:
                ...
                # otherwise show the literal [[ros:component:…]] token. The structured
                # `components` list is still returned for richer consumers.
                from ros.tools.components import strip_component_markers
    ...
    async def _write_trace(self, session, run: Run, tracer: ROSTracer, *, status: str, values: dict | None = None):
    ...
    async def _finalize(self, session, run: Run, tracer: ROSTracer, snapshot, *, status: str) -> None:
    ...
    async def _finalize_error(self, session, run: Run, tracer: ROSTracer, error: str, snapshot=None) -> None:
    ...
    async def _resume_hitl_timeout_default(self, run_id: str, tenant_id: str) -> bool:
        ...
        if self.checkpointer is None:
            return False
        try:
            from ros.models import HandoffRequest
            from ros.services.handoff import HandoffService, interrupt_hitl_meta
    ...
    async def _notify_hitl_timeout(session, run_id: str, tenant_id: str, workflow_id: str) -> None:
        ...
        from ros.models import Channel, HandoffRequest
        from ros.services.handoff import HandoffService, _channel_reply_context, _deliver
