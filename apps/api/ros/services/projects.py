...
from ros.knowledge.embeddings import KNOWN_EMBEDDING_DIMS
...
class ProjectService:
    ...
    async def create(
        ...
        *,
        ...
        # Provision the platform's built-in tools so a brand-new project has them immediately
        # (agents can reference them before the tools screen is ever opened).
        from ros.services.tools import ToolService
        await ToolService.ensure_builtins(session, tenant_id, project.id)
        return project
    ...
    async def delete(session: AsyncSession, project: Project, *, checkpointer: Any = None) -> None:
        ...
        for dim in sorted(KNOWN_EMBEDDING_DIMS):
            for prefix in ("ros_kb", "ros_qa", "ros_mem", "ros_cache"):
                try:
