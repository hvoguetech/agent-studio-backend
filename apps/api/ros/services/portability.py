...
from ros.models import Agent, AuthProvider, Component, Project, Tool, ToolSet, Workflow
...
class PortabilityService:
    ...
    async def export(
        ...
        # Built-ins are project defaults (auto-provisioned everywhere) - never travel in a bundle.
        if entity_type == "tool":
            ordered = [r for r in ordered if r.kind != "builtin"]
        ...
        # For tools, also carry the tool sets (classification) that group the exported tools, so an
        # import re-creates the grouping. Each set lists only the subset of its members that are in
        # this bundle (ids remap on import); a set with no exported members is omitted.
        if entity_type == "tool":
            exported_ids = {r.id for r in ordered}
            members = await ToolSetService.members_map(session, tenant_id, project_id)
            sets_out: list[dict] = []
            for ts in await ToolSetService.list(session, tenant_id, project_id):
                subset = [tid for tid in members.get(ts.id, []) if tid in exported_ids]
                if subset:
                    sets_out.append({"name": ts.name, "description": ts.description or "", "tool_ids": subset})
            bundle["toolsets"] = sets_out
        return bundle
    ...
    async def import_bundle(
        ...
        *,
        ...
        # Built-ins are project defaults - already present here, so never re-create them from an
        # (older) bundle that happened to include them. Dropped up front so they don't count as skips.
        builtins_dropped = 0
        if entity_type == "tool":
            kept = [it for it in items if not (isinstance(it, dict) and it.get("kind") == "builtin")]
            builtins_dropped = len(items) - len(kept)
            items = kept
        ...
        # Re-create the tool sets (classification) carried in the bundle, wiring each to the tools
        # that were just imported (bundle ids remapped to the new ids). A set whose members weren't
        # all imported still forms, with the subset that was; a set with none is skipped. Names
        # auto-rename on collision, mirroring how the items themselves are imported.
        toolsets_imported = 0
        if entity_type == "tool":
            bundle_sets = bundle.get("toolsets") or []
            if isinstance(bundle_sets, list) and bundle_sets:
                used_set_names: set[str] = set(
                    (
                        await session.execute(
                            select(ToolSet.name).where(
                                ToolSet.tenant_id == tenant_id, ToolSet.project_id == project_id
                            )
                        )
                    ).scalars().all()
                )
                for ts in bundle_sets:
                    if not isinstance(ts, dict):
                        continue
                    mapped = [id_map[t] for t in (ts.get("tool_ids") or []) if t in id_map]
                    if not mapped:
                        continue  # none of this set's tools were part of the import
                    set_name = _unique_name(str(ts.get("name") or "toolset"), used_set_names)
                    await ToolSetService.create(
                        session, tenant_id, project_id,
                        name=set_name, description=str(ts.get("description") or ""), tool_ids=mapped,
                    )
        ...
        if builtins_dropped:
            warnings.append(
                f"Skipped {builtins_dropped} built-in tool(s) — they're project defaults, already available here."
            )
    ...
    async def _create_tool(session, tenant_id, project_id, item, name, author, valid_auth):
        ...
        errors = validate_against_id({**cfg, "name": name, "kind": kind}, "ros/tool")
        if errors:
