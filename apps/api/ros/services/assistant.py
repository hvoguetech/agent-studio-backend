"""ROS Assistant - a meta-agent whose tools are the ROS platform itself.
...
from ros.engine.models import default_model_for_credentials, resolve_model
from ros.models import Agent, AuthProvider, KbSource, Project, QaPair, Tool, Workflow
...
from ros.util.serialize import content_to_text, jsonable
...
def _canvas(nodes: list[dict], edges: list[dict]) -> dict:
    ...
    return {
        "nodes": [
            {"id": n["id"], "type": "ros", "position": n["position"],
...
def build_assistant_tools(tenant_id: str, project_id: str, mutated: list) -> list:
    ...
    async def get_node_schema(node_type: str) -> str:
        ...
        from ros.engine.registry import NODE_REGISTRY
        from ros.schemas import contracts
...
SYSTEM_PROMPT = """You are the ROS Assistant - an expert copilot embedded in ROS, a platform for \
building, testing, and shipping AI agents and workflows. You help the user build INTO their current project \
by calling your tools, and you explain how ROS works.
...
How ROS concepts work, so you can explain them:
...
class AssistantService:
    ...
    async def _build_agent(*, tenant_id: str, project_id: str, checkpointer, mutated: list):
        ...
        # and HITL on top of the ROS tools - so it can take a multi-step request,
        ...
        from ros.config import settings as _settings
        ...
        # The ros-platform skill (progressive disclosure: name+description at startup,
    ...
    async def _persist_trace(
        *, tenant_id: str, project_id: str, thread_id: str | None, tracer,
        ...
        shown as a 'System' conversation in the Traces view (the ROS Assistant is internal)."""
        import uuid as _uuid
        ...
        *, tenant_id: str, project_id: str, messages: list[dict],
        ...
        try:
            async for frame in AssistantService._run(agent, {"messages": lc_messages}, config, mutated):
                yield frame
            try:
                from ros.services.runs import _last_ai_text
    ...
    async def resume(
        *, tenant_id: str, project_id: str, thread_id: str, decision: str = "approve",
        ...
        # If the paused turn had SEVERAL approval-gated tool calls, the middleware needs one
        # decision per call - expand the single UI decision to match (see _expand_hitl_decisions).
        from ros.services.runs import _expand_hitl_decisions

        value: Any = {"decisions": [{"type": decision if decision in ("approve", "reject", "edit", "respond") else "approve"}]}
        value = await _expand_hitl_decisions(agent, config, value)
        try:
