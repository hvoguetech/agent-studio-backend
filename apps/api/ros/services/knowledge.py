...
from ros.knowledge.embeddings import KNOWN_EMBEDDING_DIMS, cosine, resolve_embedder
from ros.knowledge.splitter import chunk_text
from ros.knowledge.store import Hit, _where, make_store
from ros.models import KbSource, Project, QaPair
from ros.secrets.store import SecretStore
from ros.util.http import shared_async_client

log = logging.getLogger("ros.knowledge")
...
class KnowledgeService:
    ...
    @staticmethod
    def _store(embedder):
        return make_store(collection=f"ros_kb_{embedder.dim}")
    ...
    async def embedding_health(session, tenant_id: str, project_id: str) -> dict:
        ...
        from ros.knowledge.embeddings import _MODEL_DIMS
    ...
    def _qa_store(embedder):
        ...
        return make_store(collection=f"ros_qa_{embedder.dim}")
    ...
    async def ingest(session, src: KbSource) -> KbSource:
        ...
        try:
            ...
            if src.kind in ("text", "file"):
                ...
                from ros.knowledge.crawl import (
    ...
    async def reingest(session, src: KbSource) -> KbSource:
        ...
        try:
            ...
            pass
        for collection in _dim_collections("ros_kb"):
            try:
    ...
    async def run_ingest_bg(tenant_id: str, source_id: str, *, reingest: bool = False) -> None:
        ...
        fire-and-rost background task. Embedding a real model takes seconds to tens of
    ...
    async def delete_source(session, src: KbSource) -> None:
        # Vectors are stored in a dimension-keyed collection (ros_kb_<dim>), so the
        # delete MUST target that same collection - a bare make_store() hits the
        # default `ros_kb` collection and silently leaves the real chunks behind
    ...
    async def search(
        ...
        if rerank:
            from ros.knowledge.rerank import arerank_hits
    ...
    async def update_qa(session, qa: QaPair, **changes) -> QaPair:
        ...
        for collection in _dim_collections("ros_qa"):
            try:
