...
SQLite (aiosqlite) by default; set ROS_DATABASE_URL to a Postgres async URL in
...
# Set it per-transaction from the request-scoped tenant contextvar (ros.db.scoping) via
...
if not _is_sqlite:
    ...
    def _apply_tenant_guc(conn):  # pragma: no cover - only exercised against Postgres
        try:
            from ros.db.scoping import current_tenant
...
async def init_db() -> None:
    ...
    from ros import models  # noqa: F401
