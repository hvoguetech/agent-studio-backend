...
`seed_demo_data` (only when ROS_SEED_DEMO=true) populates the showcase project,
...
from ros.models import AuthProvider, Project, Tenant, Tool, User, Workflow
...
async def bootstrap(session) -> str:
    ...
    The owner's password comes from ROS_BOOTSTRAP_ADMIN_PASSWORD when set, so the
    ...
    from ros.security import hash_password
