...
from ros.db.base import Base, PkTimestamp
...
class Agent(PkTimestamp, Base):
    ...
    project_id: Mapped[str] = mapped_column(String(36), index=True)
    name: Mapped[str] = mapped_column(String(120))
    config: Mapped[dict] = mapped_column(JSON, default=dict)  # validated vs ros/nodes/agent
...
class EntityVersion(PkTimestamp, Base):
    ...
    per (entity_type, entity_id). See ros.services.versions."""
...
class ApiKey(PkTimestamp, Base):
    ...
    is per-tenant and per-role, unlike the single static ROS_SERVICE_API_TOKEN."""
    ...
    # prefix ros_pat_, minted by ApiKeyService.create_personal) used to authenticate an
