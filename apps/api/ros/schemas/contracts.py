...
The doc's `$id`/`$ref` use relative-looking ids ("ros/common"). Standard URI
resolution would mangle nested refs (e.g. from base "ros/nodes/agent", the ref
"ros/common" resolves to "ros/nodes/ros/common"). We avoid that by rewriting
every `$id` and every "ros/..."-prefixed `$ref` to an absolute base URL on load,
...
BASE = "https://ros.dev/schemas/"


def _absolutize(node: Any) -> Any:
    """Rewrite $id and ros/* $ref values to absolute URIs (recursive)."""
    if isinstance(node, dict):
        out: dict[str, Any] = {}
        for k, v in node.items():
            if k == "$id" and isinstance(v, str) and not v.startswith("http"):
                out[k] = BASE + v
            elif k == "$ref" and isinstance(v, str) and v.startswith("ros/"):
                out[k] = BASE + v
            else:
                out[k] = _absolutize(v)
        return out
...
def _load() -> tuple[Registry, dict[str, dict], dict[str, dict]]:
    ...
    root = Path(settings.schemas_dir) / "ros"
    if not root.exists():
    ...
    mw = raw_by_id.get(BASE + "ros/middleware", {})
...
def node_schema_ref(node_type: str) -> dict:
    ...
    the type - e.g. webhook_in -> ros/nodes/trigger_webhook - still resolve)."""
    from ros.engine.registry import NODE_REGISTRY

    spec = NODE_REGISTRY.get(node_type)
    schema_id = spec.schema_id if spec else f"ros/nodes/{node_type}"
...
def raw_schema(schema_id: str) -> dict | None:
    """The raw (absolutized) schema document for an id like 'ros/nodes/router'."""
...
def validate_against_id(instance: Any, schema_id: str) -> list[dict]:
    """Validate against a registered schema id, e.g. 'ros/workflow'."""
