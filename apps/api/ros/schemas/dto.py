...
class UserConnectionIn(BaseModel):
    """A per-user downstream credential stored in ROS so a tool can act as that end user - without
    the MCP/session token being passed through. Minimally an access token; refresh_token + expires_at
    (epoch seconds) enable ROS's automatic refresh (oauth2_authorization_code). Used both by the
...
class ProjectRunIn(BaseModel):
    ...
    per-user secrets travel out-of-band in the X-ROS-Context header, never in this body.
...
class ImportIn(BaseModel):
    ...
    # Tool bundles also carry the tool sets (classification) grouping the exported tools.
