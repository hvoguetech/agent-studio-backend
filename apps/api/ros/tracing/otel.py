"""OpenTelemetry export of ROS run spans (GenAI semantic conventions).

When `ROS_OTEL_ENABLED=true` (+ an OTLP endpoint), each run's spans are exported so
ROS interoperates with standard tracing backends (Jaeger, Tempo, Honeycomb) and
...
log = logging.getLogger("ros.otel")
...
def configure(exporter=None) -> bool:
    ...
    _tracer = _provider.get_tracer("ros")
    return True
...
def export(records, *, trace_name: str = "run") -> None:
    ...
    try:
        ...
        for r in records:
            ...
            span.set_attribute("ros.kind", kind or "node")
            if model:
            ...
            if getattr(r, "cost_usd", 0):
                span.set_attribute("ros.cost_usd", r.cost_usd)
