...
from ros.model_catalog import catalog_prices

log = logging.getLogger("ros.pricing")
...
# ARE offered come from ros.model_catalog (single source of truth) and are merged below - so
    ...
    *, cache_read_tokens: int = 0, cache_creation_tokens: int = 0,
    ...
    if not rate:
        if model not in _warned and not model.startswith("fake"):
            _warned.add(model)
            log.warning("No pricing entry for model %r - its runs report $0 cost. Add it to ros/tracing/pricing.py.", model)
        return 0.0
