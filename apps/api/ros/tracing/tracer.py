"""ROSTracer - a LangChain callback handler that records spans for a run.
...
def _subagent_type(tool_input: Any) -> str | None:
    """If a tool call is a deep-agent sub-agent dispatch, return the sub-agent's name. deepagents'
    `task` tool carries `{'subagent_type': '<name>', 'description': ...}`; the callback may hand us
    that as a dict or its string repr, so handle both. Returns None for ordinary tool calls."""
    data = tool_input
...
# The run's active tracer, bound per asyncio context when a ROSTracer is constructed.
...
_ACTIVE_TRACER: ContextVar[Any] = ContextVar("ros_active_tracer", default=None)
...
def embedding_span(model: str | None, *, n_texts: int = 1, input_tokens: int = 0, name: str | None = None):
    ...
    sites (knowledge/memory RAG) can wrap embed calls unconditionally. See ROSTracer.embedding_span."""
...
class SpanRecord:
    ...
    # came back (response or error). Populated from the tool-I/O context var; see ros.tracing.tool_io.
    input: Any = None
    output: Any = None
...
class ROSTracer(BaseCallbackHandler):
    ...
    def __init__(self, event_sink: Any = None) -> None:
        ...
        # Optional live activity sink (a thread-safe callable, e.g. deque.append): receives a
        # small frame per tool / sub-agent start & end so the run stream can show a live timeline
        # of "where it routed" (the DB spans are the durable, post-run record).
    ...
    def _emit(self, frame: dict) -> None:
        ...
        except Exception:  # noqa: BLE001 - live activity is best-effort; never break a run
            pass
    ...
    def on_tool_start(self, serialized, input_str, *, run_id, parent_run_id=None, **kw):
        ...
        # A deep-agent dispatching to a sub-agent is a tool call (deepagents' `task` tool) whose
        # args carry `subagent_type`. Surface it as a named `subagent` span so the trace waterfall
        # reads e.g. "subagent · researcher" instead of a generic "tool · task".
        sub = _subagent_type(input_str)
        kind = "subagent" if sub else "tool"
        self._open(run_id, parent_run_id, f"{kind} · {sub or name}", kind, tool=name)
        self._emit({"phase": "start", "id": str(run_id), "kind": kind,
                    "name": sub or name, "parent": self._resolve_parent(parent_run_id)})
    ...
    def on_tool_end(self, output, *, run_id, **kw):
        ...
        # Carry the tool/sub-agent's result so the live UI can show it (e.g. light up a sub-agent
        # canvas node with its output on hover). Content-only + clipped so the frame stays small.
        content = getattr(output, "content", output)
        self._emit({"phase": "end", "id": str(run_id), "output": tool_io.clip(content)})
        self._close(run_id)

    def on_tool_error(self, error, *, run_id, **kw):
        self._tool_io(run_id)
        self._emit({"phase": "end", "id": str(run_id), "error": True})
        self._end_error(run_id, error)

    def _tool_io(self, run_id, *, fallback_output=None) -> None:
        """Merge the tool's captured framed I/O (set by the tool via ros.tracing.tool_io)
