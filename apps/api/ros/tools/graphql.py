...
async def execute_graphql(
    ...
    # Resolve {{env.*}} (per-environment endpoint from ROS_TOOL_VARS) and {{ctx.*}} in the
    # endpoint, so the same GraphQL tool row works across dev/qa/prod. env is fail-loud.
    endpoint = render_template(cfg["endpoint"], {"env": settings.tool_vars, "ctx": context or {}}, strict_ns=_STRICT_NS)
    await validate_url(endpoint, policy)
    ...
    try:
        if follow:
            ...
            r = await guarded_request(
                client, "POST", endpoint, policy=policy, skip_verify=skip_verify, follow_redirects=True, **kw
            )
        else:
            r = await select_client(endpoint, skip_verify=skip_verify, policy=policy, override=client).post(
                endpoint, **kw
            )
