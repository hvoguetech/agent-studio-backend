...
from ros.tools.projection import cap_payload, project_response
from ros.tracing import tool_io
from ros.util.http import select_client
from ros.util.ratelimit import rate_limiter
from ros.util.ssrf import EgressPolicy, guarded_request, validate_url

_PY = {"string": str, "integer": int, "number": float, "boolean": bool, "object": dict, "array": list}

# Namespaces whose missing keys must FAIL the call (not render empty). `{{env.*}}` resolves from
# settings.tool_vars (ROS_TOOL_VARS) - a per-environment endpoint value - so a template that
# references an undefined env var should error loudly rather than send a broken URL. ctx/input
# stay lenient (a missing per-run value is legitimately dropped).
_STRICT_NS = ("env",)
...
def _redirect_info(r: httpx.Response, followed: bool) -> dict | None:
    ...
    return {
        ...
        "location": location,
        "note": (
            "The API returned an HTTP redirect that ROS did not follow. 'location' is "
...
def _capture_tool_io(
    ...
    ROSTracer can attach it to the tool span. Best-effort: tracing must never break a call."""
...
async def execute_rest(
    ...
    *,
    ...
    # {{ctx.*}} (per-run injected values + end_user) and {{env.*}} (per-environment endpoint
    # values from ROS_TOOL_VARS). env is fail-loud (_STRICT_NS); ctx is lenient.
    ctx_vars = {"ctx": context or {}, "env": settings.tool_vars}
    ...
    for f in fields:
        ...
        if d is None:
            continue
        rendered = render_template(d, ctx_vars, strict_ns=_STRICT_NS) if isinstance(d, str) else d
    ...
    for h in req.get("headers", []) or []:
        hv = render_template(h.get("value", ""), ctx_vars, strict_ns=_STRICT_NS)
        if hv is not None:
            headers[h["name"]] = hv
    ...
    try:
        ...
        # Record the FRAMED request + response for the trace (see ros.tracing.tool_io).
