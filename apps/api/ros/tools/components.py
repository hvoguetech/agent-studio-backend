...
from ros.tools.materialize import entitlement_denial
...
COMPONENT_MARKER_RE = re.compile(r"\[\[ros:component:[A-Za-z0-9_-]+\]\]")


def component_marker(instance_id: str) -> str:
    return f"[[ros:component:{instance_id}]]"


def strip_component_markers(text: str) -> str:
    """Remove component markers from a reply for text-only surfaces (email/webhook/SMS),
    which can't render a widget and would otherwise show the literal `[[ros:component:…]]`.
