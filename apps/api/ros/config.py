...
# Repo layout: apps/api/ros/config.py -> parents[3] == repo root. In the container the
# tree is flattened to /app/ros, so parents[3] doesn't exist; fall back to API_ROOT (=/app).
...
# (/app/packages/schemas) resolves with no ROS_SCHEMAS_DIR override. Set ROS_SCHEMAS_DIR
...
class Settings(BaseSettings):
    ...
    def _parse_tool_vars(cls, v: object) -> dict[str, str]:
        ...
        if not isinstance(parsed, dict):
            raise ValueError('ROS_TOOL_VARS must be a JSON object, e.g. {"api_base":"https://..."}')
    ...
    database_url: str = Field(
        default_factory=lambda: f"sqlite+aiosqlite:///{(_DEFAULT_DATA_DIR / 'ros.db').as_posix()}"
    )
    ...
    # what you register with each OAuth provider, e.g. https://ros.yourco.com.
    public_base_url: str = "http://localhost:8000"
    # Public URL of the web console (where the SPA is served). Used to build invite links
    # emailed to new teammates, e.g. https://app.ros.yourco.com.
    public_console_url: str = "http://localhost:3000"
    # Expose ROS's MCP server as an OAuth 2.1 resource + authorization server (MCP authorization
    ...
    # token from THIS run-context key (sent by a server-to-server caller in `X-ROS-Context`). Lets
    ...
    # Empty/None = off (per-provider `token_ctx_key` only). Env: ROS_DEFAULT_TOKEN_CTX_KEY.
    ...
    smtp_from: str = "ROS <no-reply@ros.local>"
    # The seeded workspace owner. The default password lets you log in straight away in dev
    # (email: you@ros.local · password: ros-admin); CHANGE IT in production (the prod
    # guard rejects the default password).
    bootstrap_admin_email: str = "you@ros.local"
    bootstrap_admin_password: str | None = "ros-admin"
    ...
    # Env: ROS_TOOL_VARS='{"api_base":"https://api.example.com"}'. Blank/unset = {}.
    ...
    # Best-effort IPv6/AAAA-lookup suppression fallback (see ros.util.netfix). The AUTHORITATIVE
    ...
    # var (e.g. a local .venv run). Disable (ROS_PREFER_IPV4_EGRESS=false) for an IPv6-only egress
    ...
    # `scheduler_leader` lets you ship the same image everywhere and set ROS_SCHEDULER_LEADER
    # =false on the non-leaders. Set ROS_ENABLE_SCHEDULER=false to disable entirely.
    ...
    # `ros.execution_backends` entry-point group (imported lazily). The MIT core ships
    ...
    # workspace and create projects yourself. Set ROS_SEED_DEMO=true to populate.
    ...
    # "postgres", set ROS_CHECKPOINT_POSTGRES_URL (or it falls back to database_url). ---
    ...
    # misconfigured/typo'd ROS_ENVIRONMENT fails CLOSED rather than silently skipping
    # every guard.
    ...
    def validate_production(self) -> list[str]:
        ...
        if self.jwt_secret in ("", "dev-insecure-change-me"):
            problems.append("ROS_JWT_SECRET is unset/default - set a strong random secret.")
        if not self.auth_required:
            problems.append("ROS_AUTH_REQUIRED must be true outside local development.")
        if self.bootstrap_admin_password == "ros-admin":
            problems.append("ROS_BOOTSTRAP_ADMIN_PASSWORD is the dev default - set a real one.")
        if not self.egress_block_private:
            problems.append("ROS_EGRESS_BLOCK_PRIVATE must stay true outside dev (SSRF guard).")
        if self.database_url.startswith("sqlite"):
            problems.append("SQLite is not supported outside dev - set a Postgres ROS_DATABASE_URL.")
        ...
        if not self.trusted_hosts:
            problems.append("ROS_TRUSTED_HOSTS must list the API's public hostname(s) outside dev.")
        ...
        for name, url in (("ROS_PUBLIC_BASE_URL", self.public_base_url),
                          ("ROS_PUBLIC_CONSOLE_URL", self.public_console_url)):
        ...
        if self.service_api_token and len(self.service_api_token) < _MIN_SERVICE_TOKEN_LEN:
            problems.append(
                f"ROS_SERVICE_API_TOKEN is set but shorter than {_MIN_SERVICE_TOKEN_LEN} chars - "
    ...
    def startup_warnings(self) -> list[str]:
        ...
        if self.jwt_secret == "dev-insecure-change-me":
            warns.append("JWT secret is the built-in dev default - tokens are rosable. Set ROS_JWT_SECRET.")
        if self.bootstrap_admin_password == "ros-admin":
            warns.append("Bootstrap admin password is the dev default. Set ROS_BOOTSTRAP_ADMIN_PASSWORD.")
        if not self.auth_required:
        ...
        if self.enable_mcp_stdio:
            warns.append(
                ...
                "(arbitrary command execution). Restrict ROS_MCP_STDIO_ALLOWED_COMMANDS."
            )
        if self.environment.lower() not in (*self._DEV_ENVIRONMENTS, "production", "prod", "staging"):
            warns.append(f"Unrecognized ROS_ENVIRONMENT={self.environment!r} - treated as security-enforced.")
        ...
        if (self.execution_backend or "").lower() in ("freestyle", "sandbox") and self.freestyle_service_url and not self.redis_url:
            warns.append(
                f"ROS_EXECUTION_BACKEND={self.execution_backend} with a control service but no "
                "ROS_REDIS_URL - VM/sandbox-driven runs still execute and finalize, but master cannot "
                "RELAY their live stream to clients (interactive SSE degrades to 'not streaming on "
                "this server'). Set ROS_REDIS_URL to the shared Redis so the run relay works."
            )
