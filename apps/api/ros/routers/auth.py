...
from ros.util.mailer import send_email
...
async def _send_invite_email(*, to: str, link: str, inviter: str | None, role: str) -> bool:
    ...
    subject = "You've been invited to ROS"
    body = (
        f"{who}invited you to join their ROS workspace as a {role}.\n\n"
        f"Set your password and get started:\n{link}\n\n"
        "This link expires in 7 days. If you weren't expecting this, you can ignore this email."
    )
    html = (
        f"<p>{who}invited you to join their ROS workspace as a <b>{role}</b>.</p>"
...
async def request_password_reset(body: PasswordResetRequestIn, request: Request,
    ...
    if user and user.status != "disabled":
        ...
        sent = await _send_link_email(to=user.email, subject="Reset your ROS password",
...
async def request_email_verification(request: Request, user: CurrentUser = Depends(get_current_user)):
    ...
    sent = await _send_link_email(to=user.email, subject="Verify your ROS email",
                                  intro="Confirm this email address for your ROS account.",
