"""OAuth 2.1 authorization + resource server for ROS's MCP endpoint (MCP authorization spec).
...
acts AS that user (ros.routers.mcp_server._oauth_end_user).
...
# Branded consent page - mirrors the console's login card (ROS wordmark, accent-orange
# button, card on a tinted background) so the OAuth login reads as ROS, while staying a
...
<meta name="viewport" content="width=device-width,initial-scale=1"><title>Authorize %%NAME%% · ROS</title>
<style>
...
<form class="card" method="post" action="/v1/oauth/authorize">
  <div class="brand">ROS</div>
  <div class="sub"><b>%%NAME%%</b> wants to access your ROS tools over MCP, acting as you.</div>
  %%ERR%%
  %%HIDDEN%%
