...
from ros.util.ratelimit import idempotency, rate_limiter
from ros.util.tasks import spawn

log = logging.getLogger("ros.hooks")
...
def _verify_hmac_sha256(secret: str, request: Request, body: bytes) -> bool:
    ...
    signature = request.headers.get("x-ros-signature") or request.headers.get("X-Hub-Signature-256")
    if not signature:
        return False
...
async def inbound_webhook(
    ...
    if wait:
        ...
        return result
    # fire-and-rost: ack immediately, run in a TRACKED background task so failures are
