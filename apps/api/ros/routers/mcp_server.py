"""Expose a project's tools as an MCP server, over two transports that share one core.
  ...
  - **Streamable HTTP** (MCP spec 2025-03-26+): the transport native clients (Claude Desktop,
    ...
    request, no server-side session state — which is exactly ROS's model: every request is
...
from ros.models import Project, User
from ros.security import TokenError, decode_token
from ros.services.apikeys import ApiKeyService, looks_like_pat
...
try:
    ...
    # ROS already enforces trusted-hosts + https public URLs at the app layer in prod, so the
...
log = logging.getLogger("ros.routers.mcp_server")
...
def _workflow_tool_name(cfg: dict) -> str | None:
    ...
    (project.config.mcp_expose_workflow). Lets an external MCP client invoke a whole ROS
...
def _capability_tools(cfg: dict, ctx, toolset_slug: str | None) -> list:
    ...
    if not kn:
        return []
    from ros.tools.builtin import build_knowledge_capability_tools
...
def _exposed_names(ctx, sets: list, toolset_slug: str | None, excluded_ids: set[str]) -> set[str]:
    ...
    ROS-side grouping we flatten here. The surface is the enabled tools of EXPOSED sets, MINUS
...
def _session_end_user(token: str | None, proj: Project) -> dict | None:
    """The verified end_user carried by a ROS session token scoped to THIS project, or None.
...
async def _pat_end_user(token: str | None, proj: Project) -> dict | None:
    """The end_user for a per-user Personal Access Token (ros_pat_) presented to a project's MCP
...
async def _oauth_end_user(token: str | None, proj: Project) -> dict | None:
    """The end_user for a ROS-issued OAuth 2.1 MCP access token (see ros.routers.mcp_oauth),
...
async def _authorize(request: Request, proj: Project, cfg: dict) -> dict | None:
    ...
    Shared by both transports. Two accepted credential kinds:
      ...
      - a per-user credential (project-scoped session token, ros_pat_ PAT, or OAuth 2.1 access
...
async def _dispatch(request: Request, proj: Project, cfg: dict, surface: _Surface, rc, end_user: dict | None, name: str, args: dict) -> _CallResult:
    ...
    if surface.wf_tool and not surface.toolset_slug and name == surface.wf_tool:
        from ros.routers.project_run import _configured_workflow
...
def _ros_lowlevel_server(request: Request, proj: Project, cfg: dict, *, toolset_slug: str | None, end_user: dict | None, rc):
    """A per-request MCP `Server` whose tools/list + tools/call resolve ROS's surface lazily
    ...
    name = f"ros-{proj.slug or proj.id}" + (f"-{toolset_slug}" if toolset_slug else "")
...
async def _handle(project_id: str, request: Request, *, toolset_slug: str | None):
    ...
    # Ephemeral per-run context (X-ROS-Context): injected into tools as {{ctx.*}} for on-behalf-of
    ...
    if _wants_stream(request):
        ...
        server = _ros_lowlevel_server(request, proj, cfg, toolset_slug=toolset_slug, end_user=end_user, rc=rc)
    ...
    if method == "initialize":
        name = f"ros-{proj.slug or project_id}"
        if toolset_slug:
