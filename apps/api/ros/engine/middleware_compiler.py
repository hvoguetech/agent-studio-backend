...
from ros.engine.expressions import eval_truthy
from ros.engine.models import resolve_model

log = logging.getLogger("ros.middleware")
...
class _TenantBudgetState(AgentState):
    ...
    _ros_run_tokens: NotRequired[Annotated[int, UntrackedValue, PrivateStateAttr]]
...
class _TenantBudgetMiddleware(AgentMiddleware):
    ...
    - `max_usd_per_thread` is implemented via span-style pricing (ros.tracing.pricing.price)
...
class _SemanticCacheMiddleware(AgentMiddleware):
    ...
    wrap_model_call degrades to a pass-through (no caching) rather than crash - ROS's runtime
...
def _semantic_cache(c: dict, ctx: CompileContext) -> AgentMiddleware:
    from ros.services.semantic_cache import CACHE_DEFAULT_THRESHOLD, CACHE_DEFAULT_TTL_SECONDS
