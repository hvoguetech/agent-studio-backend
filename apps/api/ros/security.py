...
class _Revocations:
    ...
    (audit S11, finding d). In-process by default; persisted in REDIS when ROS_REDIS_URL is
    ...
    def revoke_jti(self, jti: str, *, exp: float | None = None) -> None:
        ...
        if r is not None:
            try:
                ...
                r.set(f"ros:revk:jti:{jti}", "1", ex=ttl)
                return
    ...
    def is_jti_revoked(self, jti: str | None) -> bool:
        ...
        if r is not None:
            try:
                return bool(r.exists(f"ros:revk:jti:{jti}"))
    ...
    def revoke_user_since(self, user_id: str, ts: float) -> None:
        ...
        if r is not None:
            try:
                r.set(f"ros:revk:user:{user_id}", str(int(ts)))
                return
            except Exception:  # noqa: BLE001
                pass
    ...
    def user_revoked_since(self, user_id: str | None) -> float | None:
        ...
        if r is not None:
            try:
                v = r.get(f"ros:revk:user:{user_id}")
...
def totp_provisioning_uri(secret: str, *, account: str, issuer: str = "ROS") -> str:
...
def create_session_token(*, tenant_id: str, project_id: str, end_user: dict, origins: list[str] | None = None, ttl_minutes: int = 30) -> str:
    ...
    sends it on each run so ROS trusts the identity without trusting the browser. The secret
...
def create_mcp_access_token(*, claims: dict, ttl_minutes: int = 60) -> str:
    """OAuth 2.1 access token for a ROS MCP resource. `aud` is bound to the project's canonical
