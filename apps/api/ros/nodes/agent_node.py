...
# ROS's default output style: every agent reply renders as GitHub-Flavored Markdown
...
COMPONENT_STYLE = (
    ...
    "markdown table. The tool returns a placeholder marker like [[ros:component:ID]]; copy that "
...
def _build_prompt(config: dict) -> str | None:
    # Static system prompt + ROS's default Markdown output style (+ component guidance when
...
def _dynamic_field_middleware(config: dict, ctx: CompileContext, base_prompt: str | None) -> list:
    ...
    if dp.get("enabled") and rules:
        ...
        from ros.engine.expressions import ExpressionError, eval_truthy
