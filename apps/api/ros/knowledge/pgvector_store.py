...
- One table (`kb_vectors`) holds every collection's rows; the `collection` column carries the
  dim-keyed name (e.g. ros_kb_384) the caller already uses, so different embedders never mix
...
from ros.knowledge.store import _CORPUS_CAP, Hit, _bump_version, _hybrid_fuse, _where
...
def _translate_where(where: dict | None, params: list) -> str:
    """Translate the Chroma where-dict subset ROS builds ($and/$or of {field: {$eq|$in}},
...
class PgVectorStore:
    def __init__(self, collection: str = "ros_kb") -> None:
