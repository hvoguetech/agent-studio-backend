...
def _build_lexical_index(store, where: dict, corpus_cap: int) -> tuple:
    ...
    from ros.knowledge.hybrid import build_bm25
...
def _hybrid_fuse(store, *, embedding, query: str, where: dict, top_k: int,
    ...
    from ros.knowledge.hybrid import bm25_scores, rrf_fuse
...
class ChromaStore:
    def __init__(self, path: str | None = None, collection: str = "ros_kb") -> None:
        path = path or settings.chroma_path
        # Collection is keyed by embedder dimension (e.g. ros_kb_256 / ros_kb_1536)
...
def make_store(collection: str = "ros_kb"):
    """Return the configured embedding store for `collection`, keyed by embedder dim by the
    caller (e.g. ros_kb_384). `settings.vector_backend` selects the backend: "chroma"
    ...
    if settings.vector_backend == "pgvector":
        from ros.knowledge.pgvector_store import PgVectorStore
