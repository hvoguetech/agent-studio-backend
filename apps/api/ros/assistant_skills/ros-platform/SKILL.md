---
name: ros-platform
description: Use when designing, building, debugging, or explaining ROS workflows, nodes, state, routing, middleware, tools, or knowledge - especially custom/complex shapes beyond the canned builders, AND for anything about deep agents, sub-agents, or supervisor patterns (how sub-agents are wired on the canvas), or per-environment tool endpoints ({{env.*}}).
---

# ROS platform deep guide

You are embedded in ROS, a visual builder for LangChain/LangGraph agents. Workflows are
...
- A REST `url_template` / GraphQL `endpoint` (or an auth template) may reference `{{env.<key>}}`,
  resolved from the deployment's `ROS_TOOL_VARS` map - so one tool row targets dev/qa/prod hosts
