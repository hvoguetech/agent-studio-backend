...
from ros.auth_providers.extract import extract_value
...
from ros.util.locks import KeyedLocks
...
class AuthResolver:
    ...
    async def _value_for(self, provider, cfg: dict, read, context: dict, *, shared_ref, fallback):
        ...
        # Inline per-request token (server-to-server /run forwarding via X-ROS-Context) takes
