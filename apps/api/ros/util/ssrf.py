...
ROS lets users (and the build assistant) point REST/GraphQL tools, webhooks,
`web_fetch`, URL knowledge ingestion, and auth/OAuth token fetches at arbitrary
URLs. On a hosted multi-tenant deployment that is a Server-Side Request ROSry
...
Set `ROS_EGRESS_BLOCK_PRIVATE=false` only for trusted single-tenant installs.
"""
...
async def guarded_request(
    ...
    **kwargs,
    ...
    """
    from ros.util.http import select_client  # local import: http imports nothing from ssrf
