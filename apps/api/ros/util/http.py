...
OS certificate-store load), and ROS previously built a fresh client per REST tool
...
log = logging.getLogger("ros.util.http")
...
def shared_llm_async_client() -> httpx.AsyncClient:
    ...
    ROS recompiles the workflow graph per run, so without a shared client every run builds a
...
def insecure_async_client() -> httpx.AsyncClient:
    ...
    the operator has allow-listed as a trusted internal target (ROS_EGRESS_ALLOW_PRIVATE_HOSTS)
...
def select_client(url: str, *, skip_verify: bool, policy, override: httpx.AsyncClient | None = None):
    ...
    if skip_verify:
        ...
        log.warning(
            "tls_skip_verify ignored for host %r: not on ROS_EGRESS_ALLOW_PRIVATE_HOSTS "
