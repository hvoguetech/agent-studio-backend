...
from ros.security import TokenError, decode_token, tokens_revoked_after
from ros.services.auth import AuthService, role_at_least
...
async def get_current_user(request: Request) -> CurrentUser:
    ...
    if token:
        ...
        if svc and hmac.compare_digest(token, svc):
            ...
            return CurrentUser(id="service", tenant_id=tenant_id, role="editor", email="service@ros.local")
        ...
        from ros.services.apikeys import ApiKeyService, looks_like_api_key
...
async def effective_role(user: CurrentUser, request: Request) -> str:
    ...
    if not project_id or user.is_fallback or user.id.startswith(("apikey:", "service")):
        return user.role
    from ros.models.entities import ProjectMember
...
def client_ip(request: Request) -> str | None:
    ...
    peer is a configured reverse proxy (settings.trusted_proxies); see ros.util.clientip.
...
ROS_CONTEXT_HEADER = "x-ros-context"
_MAX_RUN_CONTEXT_BYTES = 8192


def run_context(request: Request) -> dict | None:
    """Parse the `X-ROS-Context` header into a per-run context dict, or None if absent.
    ...
    """
    raw = request.headers.get(ROS_CONTEXT_HEADER)
    if not raw:
        return None
    if len(raw) > _MAX_RUN_CONTEXT_BYTES:
        raise HTTPException(413, f"{ROS_CONTEXT_HEADER} header too large")
    try:
        data = json.loads(raw)
    except ValueError as e:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"{ROS_CONTEXT_HEADER} must be a valid JSON object") from e
    if not isinstance(data, dict):
        raise HTTPException(status.HTTP_400_BAD_REQUEST, f"{ROS_CONTEXT_HEADER} must be a JSON object")
