...
Run it (needs the `workers` + `postgres` extras and ROS_REDIS_URL):

    arq ros.worker.WorkerSettings

It builds the SAME durable checkpointer the API uses, then processes `run_job` messages
enqueued by `ros.queue.enqueue_run` (webhook/schedule dispatch). Scale workers
...
log = logging.getLogger("ros.worker")
...
MAX_TRIES = 3
_DLQ_KEY = "ros:dlq:runs"      # Redis list of dead-lettered run jobs
...
async def _startup(ctx) -> None:
    from ros.main import _make_checkpointer
    from ros.services.runs import RunService
    ...
    log.info("ros worker started (checkpoint backend=%s)", settings.checkpoint_backend)
