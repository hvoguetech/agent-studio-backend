...
`FORGE_EXECUTION_BACKEND` (default `local`) selects the backend. `local` is the built-in MIT
`LocalBackend`; any other value is resolved from the `forge.execution_backends` entry-point
...
log = logging.getLogger("forge.execution")
...
def _resolve(name: str) -> ExecutionBackend:
    ...
    if key == "local":
        from forge.execution.local import LocalBackend  # built-in, MIT
    ...
    try:
        eps = importlib_metadata.entry_points(group="forge.execution_backends")
    except TypeError:  # pragma: no cover - importlib < 3.10 selection API
        eps = importlib_metadata.entry_points().get("forge.execution_backends", [])
    for ep in eps:
        if ep.name == key:
    ...
    raise RuntimeError(
        f"Unknown FORGE_EXECUTION_BACKEND={key!r}: no built-in backend and no installed "
        f"'forge.execution_backends' entry-point named {key!r}."
    )
