# ROS — Technology Stack

Every technology used in ROS, its purpose, and where it lives. Sourced from `apps/web/package.json`, `apps/api/pyproject.toml`, both `Dockerfile`s, `docker-compose.yml`, and `apps/api/ros/config.py`. Backend deps grouped `[in brackets]` are **optional extras** (installed on demand / prod); everything else is core.
...
| **Backend / API** | FastAPI 0.115 | HTTP/REST API framework, routing, dependency injection, middleware | `apps/api/ros/main.py`, `ros/routers/*` |
| **Backend / API** | Uvicorn[standard] 0.32 | ASGI server that runs the app | serve command (`uvicorn ros.main:app`) |
| **Backend / API** | sse-starlette 2.1 | Server-Sent Events streaming of run event frames (`run`/`node_start`/`messages`/`done`) | `ros/routers/runs.py` |
...
| **Config / Data** | Pydantic 2.9 | Request/response DTOs, data validation | `ros/schemas/dto.py`, models |
| **Config / Data** | pydantic-settings 2.6 | Environment-driven application settings | `ros/config.py` |
| **Config / Data** | email-validator 2.2 | Validate email fields (invites, auth) | `apps/api` |
| **Agent / Engine** | LangChain 1.3 + langchain-core | LLM orchestration primitives (messages, tools, model bindings) | `ros/engine/*` |
| **Agent / Engine** | LangGraph 1.2 | Stateful agent/workflow graph engine — compiles nodes into a runnable graph; the execution core | `ros/engine/compiler.py`, `ros/services/runs.py` |
| **Agent / Engine** | langgraph-checkpoint 4 | Checkpointer interface for durable / resumable run + HITL state | `ros/engine`, `ros/services/runs.py` |
...
| **Model Providers** | tiktoken 0.7 | Accurate token counting for the cost meter / budgets (falls back to len/4) | `ros/tracing/pricing.py` |
...
| **MCP** | fastmcp 3 | Expose ROS projects as MCP servers | `[mcp]` extra |
...
| **Persistence** | SQLAlchemy 2.0 [asyncio] | Async ORM / database access layer | `ros/models/entities.py` |
| **Persistence** | aiosqlite 0.20 | Async SQLite driver (dev default) | dev DB `.data/ros.db` |
...
| **Secrets / Auth** | python-jose[cryptography] 3.3 | Mint/verify platform JWT access + refresh tokens (with `kid` rotation) | auth layer (`ros/config.py` JWT settings) |
...
| **Background / Workers** | arq 0.26 | Async task queue + worker for offloaded run execution | `ros/worker.py`, `ros/queue.py`; `[workers]` |
| **Background / Workers** | croniter 2–6 | Evaluate cron `schedule` triggers | scheduler; `[workers]` |
| **Observability** | opentelemetry-sdk 1.20 | Emit run traces/spans (GenAI semantic conventions) | `ros/tracing/otel.py`; `[observability]` |
| **Observability** | opentelemetry-exporter-otlp-proto-http 1.20 | Export spans to an OTLP collector / Langfuse | `ros/tracing/otel.py`; `[observability]` (opt-in via `otel_enabled`) |
...
How the pieces above fit together. The browser talks to a **same-origin** Next.js proxy (`/api/ros/*`) that rewrites to the FastAPI backend; every run — whatever triggers it — funnels through one `RunService`, and one `ROSTracer` observes it.
...
flowchart TB
    ...
    subgraph Edge["Edge / web server"]
        Next["Next.js 14 server<br/>same-origin proxy: /api/ros/* to API"]
    end
    ...
    subgraph API["FastAPI backend — apps/api/ros"]
        MW["Middleware<br/>Audit · TrustedHost · CORS · rate-limit<br/>auth: JWT / service token · X-ROS-Context"]
        Routers["Routers (/v1/...)"]
        ...
        Tracer["ROSTracer callback<br/>spans: tokens · cost · latency"]
    end
...
> **Non-interactive triggers** (webhook / schedule / email) skip the browser and the SSE stream: they enter through the **Dispatch service** and call `RunService.run_to_completion()` instead of `stream()` — but the compile → LangGraph → ROSTracer → Trace/Span path is identical, which is what keeps observability consistent across every entry point.
